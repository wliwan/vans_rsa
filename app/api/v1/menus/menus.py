...
def _build_system_prompt(scan_tree: list[dict], all_paths: list[str], extra_prompt: str) -> str:
    """构建 AI 调用的 system prompt"""
    tree_desc = _build_scan_tree_description(scan_tree)
    paths_list = "\n".join(all_paths)

    prompt = f"""你是一个 Vue 前端项目的菜单配置专家。请根据 views 目录结构生成路由菜单配置。
...
## 原则
1. 根据目录名称推断合理的菜单名称（中文），不要直接用英文目录名作为菜单名
...
async def ai_analyze_views(req: MenuAIAnalyzeRequest):
    ...
    # 3. 构建 prompt 并调用 AI
    system_prompt = _build_system_prompt(scan_tree, all_paths, req.extra_prompt)
