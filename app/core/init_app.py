...
async def init_db():
    ...
    通过检查 aerich 迁移记录表是否存在来判断数据库是否已初始化：
    - 已初始化 → 跳过 init_db(safe=True)，仅执行 migrate + upgrade（避免基于当前模型
      建表后再重入旧迁移导致的 duplicate column / table already exists 冲突）。
    - 未初始化 → 完整执行 init_db + migrate + upgrade（首次部署）。
    """
    import sqlite3
    ...
    try:
        ...
        cur = conn.execute("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='aerich'")
        already_inited = cur.fetchone()[0] > 0
        conn.close()
    except Exception:
        pass
